              ...
              case 'add_e_finalizar':
                {
                  ...
                  let actionData, item;
                  try {
                    actionData = JSON.parse(buttonsResponse.selectedButtonId);
                    item = actionData.item;
                  } catch (err) {
                    console.error('[WEBHOOK-DELIVERY] ❌ Erro ao fazer parse de selectedButtonId:', err.message);
                    await supabase.from('processed_webhook_messages').update({
                      status: 'failed'
                    }).eq('message_id', messageId);
                    throw new Error('Formato inválido de buttonId');
                  }
                  if (!item) {
                    console.error('[WEBHOOK-DELIVERY] ❌ Nenhum item encontrado dentro do botão');
                    throw new Error('Item ausente no button payload');
                  }
                  console.log('[WEBHOOK-DELIVERY] ▶️ Invocando adicionar-item-carrinho com item:', item);
                  // 1️⃣ Adiciona o item no carrinho
                  const { data: addItemData, error: addItemError } = await supabase.functions.invoke('adicionar-item-carrinho', {
                    body: {
                      cliente_id: cliente.id,
                      item_identificado: {
                  ...
                  console.log('[WEBHOOK-DELIVERY] ✅ Item adicionado com sucesso:', addItemData);
                  // 2️⃣ Aguarda 500ms (garante consistência no BD)
                  console.log('[WEBHOOK-DELIVERY] ⏳ Aguardando 500ms antes de prosseguir...');
                  await new Promise((resolve)=>setTimeout(resolve, 500));
                  // 3️⃣ Recupera o pedido ativo
                  console.log('[WEBHOOK-DELIVERY] 🔍 Buscando pedido ativo para cliente', cliente.id);
                  const pedido = await getPedidoAtivo(supabase, cliente.id);
                  console.log('[WEBHOOK-DELIVERY] ✅ Pedido ativo obtido:', pedido?.id);
                  // 4️⃣ Atualiza status do pedido e gera palavra-chave
                  const palavra = gerarPalavraChave();
                  console.log('[WEBHOOK-DELIVERY] 💎 Palavra-chave gerada:', palavra);
                  const { error: updatePedidoError } = await supabase.from('pedidos').update({
                    status: 'em_andamento',
                    palavra_chave: palavra
                  }).eq('id', pedido.id);
                  if (updatePedidoError) {
                    console.error('[WEBHOOK-DELIVERY] ❌ Erro ao atualizar pedido:', updatePedidoError);
                  } else {
                    console.log('[WEBHOOK-DELIVERY] ✅ Pedido atualizado para "em_andamento" com palavra:', palavra);
                  }
                  // 5️⃣ Atualiza status do cliente
                  console.log('[WEBHOOK-DELIVERY] 🔄 Atualizando status do cliente para "finalizando_pedido"...');
                  console.log('[WEBHOOK-DELIVERY] 🔍 Cliente ID:', cliente?.id);
                  try {
                    ...
                    if (updateClienteError) {
                      console.error('[WEBHOOK-DELIVERY] ❌ Erro ao atualizar cliente:', updateClienteError);
                    } else if (!clienteAtualizado) {
                      console.warn('[WEBHOOK-DELIVERY] ⚠️ Nenhuma linha atualizada. Verifique se o cliente.id está correto.');
                    } else {
                      console.log('[WEBHOOK-DELIVERY] ✅ Status do cliente atualizado com sucesso:', clienteAtualizado.status_conversa);
                    }
                  } catch (err) {
                    console.error('[WEBHOOK-DELIVERY] 🔥 Exceção ao tentar atualizar cliente:', err.message);
                  }
                  // 6️⃣ Envia mensagem final ao usuário
                  const mensagemFinal = `✅ Pedido confirmado com sucesso!\n\nPara confirmar sua identidade e continuar, nos envie um *ÁUDIO* com a palavra:\n\n🔑 *${palavra}*`;
                  console.log('[WEBHOOK-DELIVERY] 📩 Enviando mensagem final ao cliente...');
                  await enviarResposta(key, whatsappNumber, mensagemFinal);
                  console.log('[WEBHOOK-DELIVERY] ✅ Fluxo "add_e_finalizar" concluído com sucesso.');
                  // debug extra
                  const { data: clienteAntes } = await supabase.from('clientes').select('id, phone_number, status_conversa').eq('id', cliente.id).single();
                  console.log('[DEBUG] clienteAntes:', clienteAntes);
                  break;
                }
